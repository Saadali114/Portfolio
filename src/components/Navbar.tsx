  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? "bg-[#090a0f]/90 backdrop-blur-xl border-b border-white/[0.08] shadow-lg shadow-black/30"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <a
            href="#home"
            className="flex items-center gap-2 sm:gap-2.5 group text-white font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded-lg p-1"
          ...
          >
            ...
            <span className="hidden sm:inline-block px-1.5 py-0.5 text-[10px] sm:text-[11px] font-mono font-medium text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 rounded-md">
              dev
            </span>
          </a>
          ...
          <nav className="hidden md:flex items-center gap-1 lg:gap-2">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="px-3.5 py-2 rounded-lg text-sm font-medium text-gray-300 hover:text-white hover:bg-white/[0.06] transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="hidden md:flex items-center gap-3">
            <a
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-gray-200 bg-white/[0.05] hover:bg-white/[0.1] border border-white/[0.1] hover:border-indigo-500/40 transition-all duration-200 group focus:outline-none focus:ring-2 focus:ring-indigo-400"
      ...
      {isOpen && (
        <div className="md:hidden">
          ...
          {/* Menu Drawer */}
          <div className="relative z-50 bg-[#090a0f] border-b border-white/[0.1] px-4 pt-3 pb-6 space-y-1 shadow-2xl">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                onClick={() => setIsOpen(false)}
                className="block px-4 py-3 rounded-xl text-base font-medium text-gray-200 hover:text-white hover:bg-white/[0.08] active:bg-indigo-600/20 transition-colors"
              >
                {link.name}
              </a>
            ))}
